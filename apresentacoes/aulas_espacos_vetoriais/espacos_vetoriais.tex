...
\begin{frame}
    \begin{exemplos}[Espaços vetoriais]
        ...
        \begin{align*}
            \mathcal{P}(\cp{K}) \pause = &\{ p(x) = a_nx^n + a_{n - 1}x^{n - 1} + \cdots + a_1x + a_0 \pause \mid \\ &a_i \in \cp{K}, \pause i = 0, 1, 2, \dots, n; \pause n \ge 0 \}.
        \end{align*}

    Dados $p(x) = a_nx^n + a_{n - 1}x^{n - 1} + \cdots + a_1x + a_0$ \pause e $q(x) = b_mx^m + b_{m - 1}x^{m - 1} + \cdots + b_1x + b_0$ \pause em $\mathcal{P}(\cp{K})$, \pause suponha que $n < m$ e defina:\pause
    \begin{itemize}
