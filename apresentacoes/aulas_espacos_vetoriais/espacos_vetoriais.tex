...
\begin{frame}
    \begin{exemplos}[Espaços vetoriais]
        Seja $\cp{K}$ um corpo, $\cp{K} = \rac$, $\real$ ou $\complex$. Considere o conjunto
            ...
            \]
            e defina
            \begin{itemize}[label=$\color{blue}\cmark$]
                \item $(a_1, a_2, \dots, a_n) + (b_1, b_2, \dots,b_n) = (a_1 + b_1, a_2 + b_2,\dots, a_n + b_n)$;
                \item $\alpha (a_1, a_2, \dots,a_n) = (\alpha a_1, \alpha a_2, \dots, \alpha a_n)$  e todo $(a_1, a_2, \dots, a_n) \in \cp{K}^n$.
            \end{itemize}
            para todos $(a_1, a_2, \dots,a_n)$ ,$(b_1, b_2, \dots,b_n) \in \cp{K}^n$.
...
\begin{frame}
    ...
    \begin{itemize}
        \item $(p + q)(x) = (a_nx^n + a_{n - 1}x^{n - 1} + \cdots + a_1x + a_0) + (b_mx^m + b_{m - 1}x^{m - 1} + \cdots + b_{n + 1}x^{n + 1}) = (a_n + b_n)x^n + \cdots + (a_1 + b_1)x + (a_0 + b_0)$
...
\begin{frame}
  \begin{definicao}
    ...
    Um vetor $w \in V$ é uma \textbf{combinação linear}\index{Espaço Vetorial!Combinação linear} \pause dos vetores $u_1$, \pause $u_2$, \pause \dots,
