...
\begin{frame}
    \begin{exemplos}[Bases Canônicas]
        \begin{enumerate}[label={\roman*})]
...
\begin{frame}
    Sim. \pause Nesse caso $[v]_\mathcal{B}$, onde $\mathcal{B} = \{u_1, u_2, \dots, u_n\}$, \pause e $[v]_\mathcal{C}$, onde $\mathcal{C} = \{w_1, w_2, \dots, w_n\}$, \pause estão relacionadas pela equação:\pause
    \begin{equation}
        [v]_\mathcal{B} = P[v]_\mathcal{C}\pause
    \end{equation}
    onde as colunas de $P$ \pause são os vetores de coordenadas \pause obtidos quando escrevemos os vetores de $\mathcal{C}$ \pause como uma combinação linear \pause dos vetores de $\mathcal{B}$. \pause Assim as colunas de $P$ são dadas por:\pause
    \begin{equation}
...
\begin{frame}
    \begin{definicao}
        A matriz $P$ que apareceu anteriormente \pause é chamada \textbf{matriz mudança de base}, \pause ou \textbf{matriz de transição}, \pause de $\mathcal{B}$ para $\mathcal{C}$. \pause Denotamos tal matriz por \pause $P_{\mathcal{B} \to \mathcal{C}}$.
    \end{definicao}
\end{frame}

\begin{frame}
    \begin{teorema}
        Se $P$ for a matriz de transição de uma base $\mathcal{B}$ \pause para uma base $\mathcal{C}$ \pause de um espaço vetorial $V$ de dimensão finita, \pause então $P$ é invertível \pause e $P^{-1}$ \pause é a matriz de transição \pause de $\mathcal{C}$ para $\mathcal{B}$.
    \end{teorema}
\end{frame}

\begin{frame}
    Do teorema anterior temos que se $\mathcal{B} = \{u_1, u_2, \dots, u_n\}$, \pause e $[v]_\mathcal{C} = \{w_1, w_2, \dots, w_n\}$ são bases do espaço vetorial $V$, então
    \begin{align}
        [v]_\mathcal{B} = P_{\mathcal{B} \to \mathcal{C}}[v]_\mathcal{C}\\
        [v]_\mathcal{C} = P_{\mathcal{C} \to \mathcal{B}}[v]_\mathcal{B}.
    \end{align}
\end{frame}

\begin{frame}
    \begin{tcolorbox}[colback=green!30, colframe=green!80!blue, title=Procedimento para calcular $P_{\mathcal{B} \to \mathcal{C}}$\, no caso de bases de $\real^n$]
        \begin{tabular}{ll}
...
\begin{frame}
    \begin{teorema}
        Sejam $\mathcal{C} = \{u_1, u_2, \dots, u_n\}$ \pause uma base qualquer do espaço vetorial $\real^n$ \pause e $\mathcal{B} = \{e_1, e_2, \dots, e_n\}$ \pause a base canônica de $\real^n$. \pause Se os vetores da base $\mathcal{C}$ \pause forem escritos em forma de colunas, então\pause
        \[
