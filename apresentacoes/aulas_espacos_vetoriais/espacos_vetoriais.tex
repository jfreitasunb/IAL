...
\begin{frame}
    Considere um espaço vetorial $V$ sobre um corpo $\cp{K}$ de dimensão $n$. Tomando duas bases para esse espaço, digamos, $\mathcal{B} = \{u_1, u_2, \dots, u_n\}$ e $\mathcal{C} = \{w_1, w_2, \dots, w_n\}$ é possível relacionar as matrizes
    \begin{align*}
        [v]_\mathcal{B} \quad \mbox{e}\quad [v]_\mathcal{C}
    \end{align*}
    para $v \in V$?
\end{frame}
...
\begin{frame}
    \begin{definicao}
        A matriz $P$ que apareceu anteriormente é chamada \textbf{matriz mudança de base}, ou matriz de transição, de $\mathcal{B}$ para $\mathcal{C}$.
    \end{definicao}
\end{frame}
