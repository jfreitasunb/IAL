    ...
    \begin{definicao}
        ...
        \begin{enumerate}[label={\roman*})]
            \item Dizemos que $T$ é \textbf{injetora} \pause se dados $u_1$, $u_2 \in V$ \pause tais que $T(u_1) = T(u_2)$, \pause então
                $u_1 = u_2$. \pause De modo equivalente, \pause se $u_1$, $u_2 \in V$ são tais que $u_1 \ne u_2$, \pause então $T(u_1) \ne T(u_2)$.

            \vspace{.5cm}

            \item Dizemos que $T$ é \textbf{sobrejetora} \pause se $\im(T) = W$. \pause Em outras palavras, \pause $T$ é \textbf{sobrejetora} se para todo $w \in W$, \pause existe $u \in V$ tal que $T(u) = w$.
        \end{enumerate}
    \end{definicao}
\end{frame}
