...
\begin{document}
    ...
    \begin{frame}
        \begin{definicao}
            Sejam $(V, +, \cdot)$  e $(W, \oplus, \otimes)$ \pause espaços vetoriais sobre um corpo $\cp{K}$. \pause Uma função
            $T \colon V \to
    ...
    \begin{frame}
        \begin{observacoes}
            \begin{enumerate}[label={\roman*})]
                \vspace*{.25cm}
                \item Se $T \colon V \to V$ é uma transformação linear \pause de $V$ em $V$, então \pause $T$ é chamada de um
                    \textbf{operador
                    linear}.\pause

                \vspace*{1cm}

                \item Para simplificar a notação, \pause vamos adotar os mesmos símbolos para indicar a soma \pause e o produto
                    por escalar \pause nos espaços vetoriais que aparecerão no decorrer do conteúdo. \pause No entanto, deve-se estar
                    ciente que estes símbolos podem ter significados diferentes, \pause dependendo do espaço vetorial em questão.
            \end{enumerate}
        \end{observacoes}
    \end{frame}
    ...
    \begin{frame}
        \begin{lema}
            ...
            \begin{enumerate}[label={\roman*})]
                \vspace*{.25cm}
                \item $T(0_V) \pause = 0_W$, \pause onde $0_V$ e $0_W$ \pause denotam os vetores nulos de $V$ e $W$,
                    respectivamente.\pause
    ...
    \begin{frame}
        \begin{lema}
            Sejam $V$ e $W$ espaços vetoriais sobre $\cp{K}$. \pause Então uma função $T \colon V \to W$ \pause é uma transformação
            linear se, e somente se,\pause
            \[
    ...
    \begin{frame}
        \begin{teorema}
            Sejam $V$ e $W$ espaços vetoriais sobre $\cp{K}$. \pause Se $\{u_1, \pause \dots, \pause u_n\}$ \pause é uma base de
            $V$ \pause e se $\{w_1, \pause \dots, \pause w_n\} \subseteq W$, \pause então existe uma única transformação linear
            \pause $T \colon V \to W$ \pause tal que $T(u_1) = w_1$, \pause $T(u_2) = w_2$, \pause \dots, $T(u_n) = w_n$.
        \end{teorema}
    \end{frame}
...
%    \end{frame}
    ...
    \begin{frame}
        \begin{tcolorbox}[colback=green!30, colframe=green!80!blue, title=Procedimento para calcular $P_{\mathcal{B} \to
            ...
            \begin{tabular}{ll}
                Passo 1. & Montamos a matriz $[\mathcal{B}\ |\ \mathcal{C}]$.\pause\\
                \\
    ...
    %         $T^{-1} \colon V \to V$ \pause tal que para qualquer $u \in V$, \pause $T^{-1}(T(u)) = u$.
    %     \end{definicao}
    % \end{frame}
    %
    % \begin{frame}
    ...
    %         \begin{enumerate}[label={\roman*})]
    %             \vspace*{.25cm}
    %             \item Seja $I = I_V = Id = Id_V \colon V \to V$ \pause a transformação linear tal que $I(u) = Id(u) = u$ \pause
    %                    para todo $u \in V$. \pause Então $I = Id$ é chamado de \textbf{operador identidade}.\pause
    %
    %             \vspace{.75cm}
    %
